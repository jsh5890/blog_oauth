...
public class UserController {
	...
	public String kakaoCallback(String code) {
		...
		try {
			oAuthToken = obMapper.readValue(responseEntity.getBody(), OAuthToken.class);
		...
		//user오브젝트 id,password,email
		logger.info("카카오 아이디 : " + kakaoProfile.getId());
		...
		//가입자 비가입자 체크
		User registedUser = userService.회원찾기(kakaoUser.getUsername());
		logger.info("registedUser : " + registedUser);
		if(registedUser.getUsername() == null) {
			userService.회원가입(kakaoUser);
		}
		
		Authentication authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(kakaoUser.getUsername(), pakaoKey));
